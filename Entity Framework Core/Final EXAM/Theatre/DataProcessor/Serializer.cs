    ...
    public class Serializer
    {
        public static string ExportTheatres(TheatreContext context, int numbersOfHalls)
        {
            TheatreDto[] theatres = context.Theatres
                .Where(noh => noh.NumberOfHalls >= numbersOfHalls && noh.Tickets.Count >= 20)
                .ToArray()
                .Select(td => new TheatreDto
                {
                    ...
                    TotalIncome = td.Tickets
                      .Where(rn => rn.RowNumber >= 1 && rn.RowNumber <= 5).ToArray().Sum(x => x.Price),
                    Tickets = td.Tickets
                      .Where(x => x.RowNumber >= 1 && x.RowNumber <= 5).ToArray()
        ...
        public static string ExportPlays(TheatreContext context, double rating)
        {
            var plays = context.Plays
                ...
                .ToList();
            var resulXml = ConverterXml.Serialize(plays, "Plays");
            return resulXml;
